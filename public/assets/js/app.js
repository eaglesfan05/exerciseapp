$(document).ready(function(){
    ...
    function populateWorkouts(params){
      ...
      allWorkouts.forEach( (workout, idx) => {
        ...
        li.html("<span>" + moment(workout.created_at).format("LLLL") + "</span>" + workout.name);
        ul.append(li);
      });
    ...
    $("button#add-workout").on("click", function(e){
      ...
      // Save to db via api
      saveSelectedWorkout(selectedWorkout);
    ...
    function saveSelectedWorkout(selectedWorkout){
      $.ajax({
        method: "POST",
        url: "/api/workout",
